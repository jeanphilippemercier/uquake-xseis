...
for ckey in ckeys_db:
    ...
    for icc in range(1, len(xcorrs_dat)):
        ...
        pcc = xutil.pearson_coeff(xref_sig[icoda], xcur_sig[icoda])
        out.append({'coeff': pc, 'pcc': pcc, 'mx': np.max(xcur_sig)})
...
vals2 = np.array([np.mean(p['pcc']) for p in stats])
