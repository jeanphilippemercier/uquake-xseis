...
# plt.plot(times, snr)
ax = plt.gca().twinx()
plt.plot(times, sig, alpha=0.8, label='signal')
plt.plot(times, noise, alpha=0.8, label='noise')
xplot.axvline_times([coda_start_sec, coda_end_sec], alpha=0.2, color='black')
plt.xlabel("lag time (s)")
plt.legend()

plt.plot(times, np.log(sig))
xplot.axvline_times([coda_start_sec, coda_end_sec])
...
mx = np.max(xcorrs_dat_raw, axis=1) * 100
plt.plot(mx)
...
reload(xplot)
im = xplot.im_freq(xcorrs_dat, sr)
xplot.HookImageMax(im)


xplot.im_freq(xcorrs_dat, sr)

# im = xplot.im(xcorrs_dat, times=times)
im = xplot.im(xcorrs_dat_raw, norm=False, times=times)
...
nsmooth = 20
noise, c1, c2 = cc_noise_level(xcorrs_dat)
stack = np.mean(xcorrs_dat, axis=0)
envelope = xutil.envelope(stack)

# plt.plot(times, stack)
plt.plot(times, envelope)
plt.plot(times, noise)
envelope = xutil.smooth(envelope, nsmooth)
noise = xutil.smooth(noise, nsmooth)
plt.plot(times, envelope)
plt.plot(times, noise)
snr = envelope / noise
...
plt.plot(np.abs(stack))

%%timeit
np.abs(hilb)

xutil.envelope(stack)
