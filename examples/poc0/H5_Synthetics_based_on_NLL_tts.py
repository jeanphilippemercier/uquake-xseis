...
# 388036
# src_loc = np.array([1600, 1420, 1000])
# src_loc = xutil.shift_locs_ot(np.array([1600, 1420, 1000]), unshift=True)
src_loc = np.array([651600, 4767420, 200])
# xplot.plot3d(stalocs, src_loc)
dd = xutil.dist2many(src_loc, gridlocs)
isrc = np.argmin(dd)
print(np.min(dd), isrc)
...
hf = h5py.File(ddir + 'sim_nll.h5', 'w')
...
hf.create_dataset('sta_names', data=names.astype('S4'))
